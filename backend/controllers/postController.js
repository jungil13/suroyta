...
exports.createPost = async (req, res) => {
  ...
  const { title, content, category_name, destination, latitude, longitude } = req.body;
  ...
  try {
    ...
    res.status(201).json({
      ...
      post: {
        ...
        image_urls,
        destination,
        latitude, // Include latitude in response
        longitude, // Include longitude in response
      },
    });
  } catch (error) {
...
// back/controllers/postController.js
exports.getAllPosts = async (req, res) => {
  try {
    const [posts] = await db.promise().query(`
      ...
      GROUP BY p.post_id, u.username, c.name, p.user_id, u.bio  
...
exports.getPostById = async (req, res) => {
  ...
  try {
    // Fetch the post details including the author's profile photo, like count, and location data
    const [postResult] = await db.promise().query(
      `
        SELECT 
          ...
          p.content, 
          p.image_url, 
          p.latitude,  -- Include latitude
          p.longitude, -- Include longitude
          c.name AS category_name, 
          u.username AS author, 
    ...
    // Return the post details along with comments and location information
    res.status(200).json({
      ...post,
